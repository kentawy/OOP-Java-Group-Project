...
![ER-diagram](https://prnt.sc/oPnhpUbyPD-Y)
...
├── src/main/java
│   ├── org/example/oopjavagroupproject
│   │   ├── data/                # Класи DAO для роботи з таблицями БД
│   │   ├── database/            # Клас DatabaseHandler для з'єднання з БД
│   │   ├── model/               # Класи-сутності предметної області
│   │   ├── HelloApplication.java# Головний клас для запуску JavaFX
│   │   ├── HelloController.java # Контролер для обробки подій UI
│   │   ├── Launcher.java        # Клас-обгортка для коректного запуску .jar
│   │   ├── PropertiesBundleProvider.java # Утиліта для локалізації
│   │   └── UTF8Control.java     # Налаштування кодування ресурсів
│   └── module-info.java         # Модульна конфігурація Java
├── src/main/resources
│   ├── org/example/oopjavagroupproject
│   │   ├── hello-view.fxml      # FXML-розмітка графічного інтерфейсу
│   │   ├── light-theme.css      # CSS-стилі (світла тема)
│   │   ├── styles.css           # Основні CSS-стилі
│   │   ├── logo.png             # Логотип застосунку
│   │   ├── messages_en.properties # Файл локалізації (англійська)
│   │   └── messages_uk.properties # Файл локалізації (українська)
│   └── db.properties            # Конфігураційні дані для підключення до PostgreSQL
├── .gitignore                   # Файли та папки, що ігноруються Git
├── mvnw / mvnw.cmd              # Скрипти Maven Wrapper
├── pom.xml                      # Конфігурація проєкту та залежності Maven
...
## 🔗 Корисні посилання
- **📚 JavaDoc:** [Переглянути документацію проєкту](https://kentawy.github.io/OOP-Java-Group-Project/)
-
---
...
- `23.03 - 29.03.2026` [x] Проєктування схеми бази даних та генерація SQL-скриптів
- `30.03 - 04.04.2026` [x] Налаштування Maven, реалізація підключення до БД (JDBC)
- `05.04 - 15.04.2026` [x] Реалізація шару DAO та базових CRUD-операцій
- `16.04 - 21.04.2026` [x] Реалізація системи багатокритеріального пошуку
- `16.04 - 21.04.2026` [x] Створення графічного інтерфейсу (JavaFX) та зв'язка з логікою
- `21.04 - 21.04.2026` [x] Комплексне тестування, відловлювання багів, оптимізація
- `21.04 - 21.04.2026` [x] Написання JavaDoc, підготовка презентації та фінальна здача
